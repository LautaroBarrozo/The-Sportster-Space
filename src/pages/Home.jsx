...
export const Home = () => {
    ...
    const removeProductFromCart = (product) => {
        cart.map((cartProduct) => {
            if (cartProduct.id === product.id && (product.userId && cartProduct.userId) === activeUser.id) {
    ...
    const handleMinusBtnEvent = (e) => {
        const product = getProduct(e.target.getAttribute('data-id'))
        const existingCartProduct = cart.find((item) => item.id == product.id && item.userId == activeUser.id);
        ...
        <Navbar_header>
            <Navbar_menu className="center-flex">
                ...
                <User_menu id="user-info-menu" showusermenu={showusermenu}>
                    ...
                    {activeUser != 0 ? renderUserInfo(activeUser) : renderLoginButton()  }
